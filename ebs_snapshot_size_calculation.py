...
for vol in vols:
    vol_details = vol["Attachments"]
    vol_detail_len = len(vol_details)
    if vol_detail_len == 0:
        vol_state = "Not Attached"
        ...
        if snapshots:
            ...
            total_gb = 0
            num_snapshots = 0
            print('%25s %20s %25s %15s %25s %10s' % (
            "snapshot_id", "snapshot size (GB)", "change rate (%)", "storage tier", "start time", "age"))
            for snapshot in sorted_snapshots:
                ...
                if snapshot["StorageTier"] == "archive":
                    print('%25s %20s %25s %15s %25s %10s' % (
                    snapshot["SnapshotId"], "N/A", "N/A", snapshot["StorageTier"], str(snapshot['StartTime']).split('.')[0],
                    age))
                else:
                    ...
                    if vol == vol_prev and vol != 'vol-ffffffff' and snapshot["StorageTier"] != "Archive":
                        ...
                        print('%25s %20s %25s %15s %25s %10s' % (
                        snapshotid, str(round(diff, 2)), str(round(diff / snapshot["VolumeSize"] * 100, 2)),
                        snapshot["StorageTier"], str(snapshot['StartTime']).split('.')[0], age))
                        total_gb += diff

                    else:
                        initial = snapshot_size(snapshotid)
                        print('%25s %20s %25s %15s %25s %10s' % (
                        snapshotid, str(round(initial, 2)), str(round(initial / snapshot["VolumeSize"] * 100, 2)),
                        snapshot["StorageTier"], str(snapshot['StartTime']).split('.')[0], age))
                        total_gb += initial
                        vol_prev = vol
                    snapshotid_prev = snapshotid
            print("Volume " + vol + " is not attached to any instance and has " + str(num_snapshots) + " snapshots," + "with estimated consumed capacity of " + str(round(total_gb,2)) + "GB, excluding any archived snapshots.")
        else:
            print(vol["VolumeId"] + " is not attached to any instance and does not have any snapshots.")
    else:
                ...
                for snapshot in sorted_snapshots:
                    ...
                    if snapshot["StorageTier"] == "archive":
                        ...
                        if vol == vol_prev and vol != 'vol-ffffffff' and snapshot["StorageTier"] != "Archive":
                            ...
                            vol_prev = vol
                        snapshotid_prev = snapshotid
                print("Volume " + vol + " is attached to " + vol_detail["InstanceId"] + " and has " + str(num_snapshots) + " snapshots," + "with estimated consumed capacity of " + str(round(total_gb,2)) + "GB, excluding any archived snapshots.")
            else:
                print(vol["VolumeId"] + " is attached to " + vol_detail["InstanceId"] + " , but does not have any snapshots.")
